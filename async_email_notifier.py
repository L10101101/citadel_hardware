...
from config_store import get_smtp_config, get_app_config
...
def _build_inline_image(path: str, cid: str, max_size: tuple[int, int] = (256, 256), jpeg_quality: int = 68):
    try:
        from PIL import Image
        with Image.open(path) as img:
            # Shrink large logos before attaching to reduce email payload size.
...
async def send_campus_notification(guardian_email: str, student_name: str, timestamp: str, notification_type: str = "entry"):
    ...
    campus_signoff = campus_name or "Campus Administration"
...
This is to inform you that your child, {student_name}, has {action_text} University of Caloocan City - {campus_name} campus.
...
© {datetime.now().year} {campus_signoff}. All rights reserved.
"""
...
<p style="color:#fff;margin:0;font-size:15px;font-weight:bold;">{campus_signoff}</p>
</td>
...
<p>Your child, <b style="color:#064F32;">{student_name}</b>, has {action_text} University of Caloocan City - {campus_name} campus.</p>
...
<b style="color:#064F32;">{campus_signoff}</b><br>
...
<p>© {datetime.now().year} {campus_signoff}. All rights reserved.</p>
</td></tr>
    ...
    for cid, path in [("logo", logo_path), ("ucc_logo", ucc_logo_path)]:
        if path:
            try:
                img = _build_inline_image(path, cid)
                if img is None:
                    continue
                msg.attach(img)
